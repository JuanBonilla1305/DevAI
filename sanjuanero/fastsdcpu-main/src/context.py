...
from backend.models.lcmdiffusion_setting import DiffusionTask, LCMLora
    ...
    def generate_text_to_image(
        ...
        try:
            ...
            if not settings.lcm_diffusion_setting.lcm_lora:
                settings.lcm_diffusion_setting.lcm_lora = LCMLora()
            model_id = (
                settings.lcm_diffusion_setting.openvino_lcm_model_id or ""
            ).lower()
            if (
                settings.lcm_diffusion_setting.use_openvino
                and "flux2-klein" in model_id
            ):
                settings.lcm_diffusion_setting.negative_prompt = ""
