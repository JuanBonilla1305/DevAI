...
async def generate(diffusion_config: LCMDiffusionSetting) -> StableDiffusionResponse:
    ...
    if (
        ...
        init = diffusion_config.init_image
        if isinstance(init, list):
            decoded = []
            for item in init:
                image = base64_image_to_pil(item).convert("RGB")
                decoded.append(image)
            app_settings.settings.lcm_diffusion_setting.init_image = decoded
        else:
            app_settings.settings.lcm_diffusion_setting.init_image = (
                base64_image_to_pil(init).convert("RGB")
            )
