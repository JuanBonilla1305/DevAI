...
class LCMTextToImage:
    ...
    def init(
        ...
        if lcm_diffusion_setting.diffusion_task == DiffusionTask.edit_image.value:
            ...
            source_images = lcm_diffusion_setting.init_image
            if not isinstance(source_images, (list, tuple)):
                source_images = [source_images]
            prepared_images = []
            for source_image in source_images:
                image_max = min(max_size, 512) if len(source_images) > 1 else max_size
            ...
            lcm_diffusion_setting.init_image = (
                prepared_images if len(prepared_images) > 1 else prepared_images[0]
            )
            if len(prepared_images) == 1:
                lcm_diffusion_setting.image_width, lcm_diffusion_setting.image_height = (
                    prepared_images[0].size
                )
    ...
    def generate(
        ...
        if lcm_diffusion_setting.use_openvino:
            ...
            if lcm_diffusion_setting.diffusion_task == DiffusionTask.edit_image.value:
                edit_images = lcm_diffusion_setting.init_image
